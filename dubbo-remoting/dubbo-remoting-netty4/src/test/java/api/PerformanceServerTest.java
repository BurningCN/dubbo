...
public class PerformanceServerTest {
    ...
    private static ExchangeServer statServer() throws Exception {
        ...
        String url = "exchange://0.0.0.0:" + port + "?transporter="
                + transporter + "&serialization="
                + serialization + "&threadpool=" + threadpool
                + "&threads=" + threads + "&iothreads=" + iothreads + "&buffer=" + buffer + "&channel.handler=" + channelHandler;

        // exchange://0.0.0.0:9911?transporter=netty&serialization=hessian2&threadpool=limited&threads=200&iothreads=13&buffer=8192&channel.handler=execution
        // 注意这些参数在Exchangers.bind的时候是怎么用的，

        // transporter=netty --> Transports内部getTransporter().bind(url, handler)
        // serialization=hessian2 --->  CodecSupport.getxx
        // threadpool=limited ---> AbstractServer内部 executorRepository.createExecutorIfAbsent(url) --> createExecutor
        // threads=200 ---> LimitedThreadPool.getExecutor
        // iothreads=13 ---> NettyServer的workerGroup
        // channel.handler=execution ---> ChannelHandlers.wrapInternal (比如ExecutionDispatcher)

        // 上面只是显示的这么多，实际还有很多，具体可以断点打在URL的这个方法上 public String getParameter(String key) {

        ExchangeHandler exchangeHandlerAdapter = new ExchangeHandlerAdapter() {
        ...
        // Start server
        ExchangeServer server = Exchangers.bind(url, exchangeHandlerAdapter);
...
//        }
        final int port = PerformanceUtils.getIntProperty("port", 9911);
        final boolean telnet = PerformanceUtils.getBooleanProperty("telnet", false);
        if (telnet) {
            // 进去
